...
/**
 * This is a class for extracting minutiae features from a fingerprint image.
 */
...

{
	...
	//---------------------------------------------------------- CONSTANTS --//
	int DEFAULT_ZERO_COLOR = Color.BLACK;		// Default color of FALSE pixels
	int DEFAULT_ONE_COLOR = Color.LTGRAY;		// Default color of TRUE pixels
	//---------------------------------------------------------- VARIABLES --//	
	...
	int height;
	int zeroColor;							// Colors
	int oneColor;
	
	Bitmap originalImage;				// Original image
...
//			originalImage = ImageIO.read(new File(filename));
	 ...
	 */
	public void setColors(int zeroColor, int oneColor)
	{
	...
	public Bitmap toBitmap()
	{
		...
		for (int i = 0 ; i < width ; ++i)
		{
			for (int j = 0 ; j < height ; ++j)
			{
				...
				if (binMap[i][j] == false)
				{
					bufferedImage.setPixel(i, j, zeroColor);
				}
				else
				{
					bufferedImage.setPixel(i, j, oneColor);
				}
			}
		}
	 ...
	 * @return the buffered image
	 */
	public Bitmap directionToBufferedImage(direction [][] dirMatrix)
	{
		Bitmap bufferedImage = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
		...
		for (int i = 0 ; i < width ; ++i)
		{
			for (int j = 0 ; j < height ; ++j)
			{
				bufferedImage.setPixel(i, j, (dirToColor (dirMatrix[i][j])));
			}
		}
	 ...
	 */
	private int dirToColor(direction dir)
	{
		switch (dir) 
		{
			case NONE:
